...
if inv1>inv2:
    ...
    ecart2=inv1+inv2-TTS_pos[S1[len(S1)-1]]-TSS_pos[S2[0]]
    newTTS_pos[S2]=TTS_pos[S2] + ecart2
    newTSS_pos[S2]=TSS_pos[S2] + ecart2
    newBarr_fix[S2]=Barr_fix[S2] + ecart2
    
    ecart1=TTS_pos[S1[len(S1)-1]]-(inv1+inv2-TSS_pos[S2[0]])
    newTSS_pos[S1]=TSS_pos[S1]-ecart1
    newTTS_pos[S1]=TTS_pos[S1]-ecart1
    newBarr_fix[S1]=Barr_fix[S1] - ecart1
    
    if newBarr_fix[0]<0: # translation if one region is split between end and beginning
        newBarr_fix=newBarr_fix-newBarr_fix[0]
        newTSS_pos=newTSS_pos-newBarr_fix[0]
        newTTS_pos=newTTS_pos-newBarr_fix[0]
    
else:
    S=list(set(np.ndarray.tolist(np.where(inv1<TSS_pos)[0])).intersection(np.ndarray.tolist(np.where(inv2>TTS_pos)[0])))
    S2=np.copy(S)
    S.reverse()
    noms_genes[S2]=noms_genes[S]
    newstrands[S2]=-strands[S]

    ecart=inv1+(inv2-TTS_pos[S2[len(S2)-1]])-TSS_pos[S2[0]]
